...
function generateLinkedInSearchUrl(name: string, company: string): string {
...
function generateRecentPosts(
  ...
  switch (contact.category) {
    case "talent-acquisition":
      posts.push(
        `🚀 We're hiring! ${company} is looking for talented ${jobTitle}s in ${location}. Apply now!`,
        `Exciting opportunities at ${company}! DM me if you're interested in engineering roles.`,
      );
      break;
    case "hiring-manager":
      posts.push(
        `Looking to grow my team at ${company}. If you're passionate about building scalable systems, let's connect.`,
        `Just shipped a major feature with the team. Proud of what we've built at ${company}!`,
      );
      break;
    case "hr":
      posts.push(
        `${company} is one of the best places to work in India! Proud of our engineering culture.`,
        `We value diversity and inclusion at ${company}. Check out our open positions!`,
      );
      break;
    case "referral":
      posts.push(
        `Great time working at ${company}! The engineering culture here is top-notch.`,
        `If you're looking for your next role, I'd recommend checking out ${company}. Happy to refer strong candidates.`,
      );
      break;
  }
  return posts;
