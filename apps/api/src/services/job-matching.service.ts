...
export class JobMatchingService {
  ...
  async match(
    ...
    if (this.aiSidecarService.isConfigured()) {
      const sidecarResult = await this.matchWithSidecar(
        profileSkills,
        jobDescription,
      );
      if (sidecarResult) {
  ...
  private async matchWithSidecar(
    profileSkills: string[],
    jobDescription: string,
  ): Promise<MatchResult | null> {
    try {
      const response = await this.aiSidecarService.semanticMatch(
        extractSkills(profileSkills.join(" ")),
        jobDescription,
      );
      ...
      const matchScore = Math.max(
        0,
        Math.min(100, Math.round(Number(response.matchScore) || 0)),
      );
      return {
        matchScore,
        matchedSkills: this.normalizeSkillList(response.matchedSkills),
        missingSkills: this.normalizeSkillList(response.missingSkills),
        suggestedImprovements: this.normalizeStringList(
          response.suggestedImprovements,
        ),
      };
    } catch {
      return null;
  ...
  private async matchWithModel(
    ...
    try {
      ...
      const response = await this.aiChatService.completeJson([
        {
          role: "system",
          content:
            "You rank resume-to-job semantic fit. Return valid JSON with keys matchScore, matchedSkills, missingSkills, and suggestedImprovements. matchScore must be an integer from 0 to 100. Keep skills normalized to lowercase technology names. Do not include markdown fences.",
        },
        {
          role: "user",
      ...
      const suggestedImprovements = this.normalizeStringList(
        response.suggestedImprovements,
      );
  ...
  private matchWithSkills(
    ...
    const missingSkills = jobSkills.filter(
      (skill) => !normalizedProfileSkills.has(skill),
    );
  ...
  private normalizeSkillList(value: unknown) {
