...
export class MultiSourceJobProvider implements JobProvider {
  readonly sourceMode = "remote" as const;
  private readonly enrichment = new JobEnrichmentService();

  private readonly providers = this.buildProviders();

  async fetchJobs(context: JobProviderContext): Promise<ProviderJob[]> {
    const results = await Promise.allSettled(
      this.providers.map((provider) => provider.fetchJobs(context)),
    );
  ...
  private buildProviders() {
    const providers: Array<Pick<JobProvider, "fetchJobs">> = [
