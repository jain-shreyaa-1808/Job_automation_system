...
export class AutoApplyService {
  ...
  async requestApplication(input: AutoApplyInput) {
      ...
      { userId: input.userId, jobId: input.jobId },
      {
        ...
        $push: {
          history: {
            ...
            details:
              "Auto-filled from profile. Awaiting user verification before submission.",
          },
        },
      },
    ...
    return {
      ...
      preFilledData,
      message:
        "Application form has been pre-filled. Please review the details and confirm to submit.",
    };
  }
   ...
   */
  async confirmApplication(input: ConfirmApplicationInput) {
    ...
    if (!application) {
      throw AppError.notFound(
        "No pending application found. Please queue the application first.",
      );
    }
    ...
    return {
      status: "applied",
      message:
        "Application submitted successfully! The details have been confirmed and queued for processing.",
    };
  }
  ...
  private calculateExperience(
    ...
    const earliest = experience
      ...
      .filter(Boolean)
      .sort()[0];
    if (!earliest) return "0 years";
    const years = Math.max(
      0,
      new Date().getFullYear() - parseInt(earliest.slice(0, 4), 10),
    );
