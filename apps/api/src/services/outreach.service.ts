...
export class OutreachService {
  ...
  async generate(userId: string, jobId: string, recruiterLeadId?: string) {
    const [profile, job, lead] = await Promise.all([
      UserProfileModel.findOne({ userId }).lean<OutreachProfile | null>(),
      JobModel.findById(jobId).lean<OutreachJob | null>(),
      recruiterLeadId
  ...
  private async generateWithModel(params: {
    ...
    description: string;
    topSkills: string[];
    bestProject: OutreachProfile["projects"][0] | null;
    bestExperience: OutreachProfile["experience"][0] | null;
    matchedSkills: string[];
    certifications: string[];
  }) {
    ...
    try {
      ...
      const linkedinMessage =
        typeof response.linkedinMessage === "string"
          ? response.linkedinMessage.trim()
          : "";
      const referralMessage =
        typeof response.referralMessage === "string"
  ...
  private findBestProject(
    projects: OutreachProfile["projects"],
    jdKeywords: string[],
  ) {
    if (!projects.length) return null;
    return [...projects].sort((a, b) => {
      const aScore =
        (a.technologies ?? []).filter((t) =>
          jdKeywords.some((k) => t.toLowerCase().includes(k)),
        ).length +
        (a.summary
  ...
  private findBestExperience(
    experience: OutreachProfile["experience"],
    jdKeywords: string[],
  ) {
...
${senderName}`;
  ...
  private buildLinkedInMessage(params: {
    recipientName: string;
    senderName: string;
    company: string;
    jobTitle: string;
    ...
    const greeting =
      recipientName !== "Hiring Manager" ? `Hi ${recipientName},` : `Hello,`;
...
${senderName}`;
  ...
  private buildReferralMessage(params: {
    senderName: string;
    company: string;
    jobTitle: string;
    topSkills: string[];
