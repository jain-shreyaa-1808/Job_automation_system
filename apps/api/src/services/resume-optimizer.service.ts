...
type ResumeRenderData = {
  ...
  rankedProjects: ResumeProfile["projects"];
  rankedExperience: ResumeProfile["experience"];
...
export class ResumeOptimizerService {
  ...
  async generateTailoredResume(userId: string, jobId: string) {
    ...
    const heuristicRenderData = this.buildResumeRenderData(
      profile,
      job,
      atsKeywords,
    );
    const renderData = await this.refineRenderDataWithModel(
      profile,
      job,
      heuristicRenderData,
      atsKeywords,
    ...
    const atsSuggestions = this.generateAtsSuggestions(
      profile,
      job,
      atsKeywords,
    );
  ...
  private async refineRenderDataWithModel(
    ...
    try {
      ...
      return {
        ...
        summaryLine,
        orderedSkills:
          orderedSkills.length > 0 ? orderedSkills : renderData.orderedSkills,
      };
    } catch {
      return renderData;
    }
  ...
  private extractJDKeywords(description: string): string[] {
  ...
  private generateAtsSuggestions(
    ...
    const missing = job.missingSkills.filter(
      (skill) => !profileSkillsLower.has(skill.toLowerCase()),
    );

    if (missing.length > 0) {
      suggestions.push(
        `Consider adding these JD keywords if you have exposure: ${missing.join(", ")}.`,
      );
    }

    suggestions.push(
      `ATS keywords injected into your resume: ${atsKeywords.slice(0, 10).join(", ")}. Ensure they appear naturally.`,
    );
    suggestions.push(
      "Quantify achievements with concrete outcomes such as performance gains, revenue impact, or scale.",
    );
    suggestions.push(
      `Tailor your summary to emphasize ${job.title}-relevant skills over generic statements.`,
    );

    if (profile.certifications.length > 0) {
      suggestions.push(
        `Your certifications (${profile.certifications.join(", ")}) are included and aligned to ${job.company}.`,
      );
    }
  ...
  private buildResumeRenderData(
    ...
    const matchedSkills = normalizedSkills.filter((skill) =>
      atsSet.has(skill.toLowerCase()),
    );
    const otherSkills = normalizedSkills.filter(
      (skill) => !atsSet.has(skill.toLowerCase()),
    );
    const orderedSkills = [
    ...
    const summaryLine = this.limitText(
      this.normalizeSentence(
        `Results-driven engineer with hands-on experience in ${topSkills.join(", ")}. Builds scalable, customer-facing software with strong focus on ${summaryKeywords} and production quality`,
      ),
      220,
    );
    ...
    const rankedProjects = [...profile.projects].sort((left, right) => {
      const leftScore = (left.technologies ?? []).filter((technology) =>
        atsSet.has(technology.toLowerCase()),
      ).length;
      const rightScore = (right.technologies ?? []).filter((technology) =>
        atsSet.has(technology.toLowerCase()),
      ).length;
    ...
    const rankedExperience = [...profile.experience].sort((left, right) => {
      const leftText =
        `${left.title ?? ""} ${left.summary ?? ""}`.toLowerCase();
      const rightText =
        `${right.title ?? ""} ${right.summary ?? ""}`.toLowerCase();
      const leftScore = atsKeywords.filter((keyword) =>
        leftText.includes(keyword.toLowerCase()),
      ).length;
      const rightScore = atsKeywords.filter((keyword) =>
        rightText.includes(keyword.toLowerCase()),
      ).length;
      return rightScore - leftScore;
    });
    ...
    return {
      ...
      rankedProjects: rankedProjects.slice(0, 2).map((project) => ({
        ...
        summary: this.limitText(
          this.normalizeSentence(project.summary ?? ""),
          150,
        ),
      ...
      rankedExperience: rankedExperience.slice(0, 2).map((experience) => ({
        ...
        summary: this.limitText(
          this.normalizeSentence(experience.summary ?? ""),
          180,
        ),
      })),
      education: profile.education.slice(0, 1).map((education) => ({
        ...
        institution: this.normalizeListItem(
...
\\end{document}`;
  ...
  private async buildPdfResume(
    ...
    const canFit = (requiredHeight: number) =>
      cursorY - requiredHeight >= margin;
    ...
    if (
      renderData.certifications.length > 0 &&
  ...
  private normalizeListItem(value: string) {
