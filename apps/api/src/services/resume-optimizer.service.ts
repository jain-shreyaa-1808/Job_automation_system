...
export class ResumeOptimizerService {
  ...
  private generateAtsSuggestions(
    ...
    const missing = job.missingSkills.filter(
      (s) => !profileSkillsLower.has(s.toLowerCase()),
    );
    ...
    suggestions.push(
      "Quantify achievements — e.g. 'reduced latency by 40%' or 'served 1M+ requests/day'.",
    );
    suggestions.push(
      `Tailor your summary to emphasise ${job.title}-relevant skills over generic ones.`,
    );

    if (profile.certifications.length > 0) {
      suggestions.push(
        `Your certifications (${profile.certifications.join(", ")}) are included — they boost ATS score for ${job.company}.`,
      );
    }
  ...
  private buildRefinedResume(
    ...
    const matchedSkills = profile.skills.filter((s) =>
      atsSet.has(s.toLowerCase()),
    );
    const otherSkills = profile.skills.filter(
      (s) => !atsSet.has(s.toLowerCase()),
    );
    ...
    const rankedProjects = [...profile.projects].sort((a, b) => {
      const aScore = (a.technologies ?? []).filter((t) =>
        atsSet.has(t.toLowerCase()),
      ).length;
      const bScore = (b.technologies ?? []).filter((t) =>
        atsSet.has(t.toLowerCase()),
      ).length;
